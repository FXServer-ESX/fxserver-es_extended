...
module.sellDrugsNPC = function(isSelling)
  local selling

  if isSelling then
    ESX.SetInterval(0, function() -- 100ms interval (0.1 seconds)
      for ped in EnumeratePeds() do -- Peds search
        if DoesEntityExist(ped) then
          if not IsPedDeadOrDying(ped, 1) and not IsPedAPlayer(ped) then
            local playerCoords = GetEntityCoords(GetPlayerPed(-1))
            ...
            local distance = GetDistanceBetweenCoords(pedCoords.x, pedCoords.y, pedCoords.z, playerCoords.x, playerCoords.y, playerCoords.z, true)
...
module.test = function(stop)
  local asd
      
  i = 0
  asd = ESX.SetInterval(1000, function()
    i = i + 1
    print(i)
  end)
